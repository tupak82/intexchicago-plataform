...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: service.title, description: service.description, url: canonical, type: "website" },
  };
}
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className={`servicePage servicePage--${visualType}`}>
        ...
        <div className="serviceHeroStage">
          <div className="serviceHeroInner">
            ...
            <div className="serviceHeroTrust">
              <span>Chicago roofing specialists</span><span>Clear scope</span><span>Documented work</span>
            </div>
          </div>
      ...
      <section className="serviceBody">
        <div className="serviceSplit">
          <div><p className="kicker dark"><span /> Common situations</p><h2>When to call Intex</h2></div>
          <ul className="serviceList">{service.problems.map((problem) => <li key={problem}>{problem}</li>)}</ul>
        </div>
        ...
        <div className="relatedServices relatedServicesPremium">
          <div className="relatedServicesIntro">
            <div><p className="kicker dark"><span /> Connected roofing system</p><h2>One roof. Different problems. <em>One smarter next step.</em></h2></div>
          ...
          <div className="relatedGrid relatedGridPremium">
            {service.related.map((relatedSlug, index) => {
              ...
              const relatedVisualType = visualTypeForSlug(related.slug);
              return (
                <a className={`relatedCard relatedCardPremium relatedCardPremium--${index + 1}`} href={`/${related.slug}/`} key={related.slug} data-visual={relatedVisualType}>
                  <RoofingServiceVisual type={relatedVisualType} compact label={`${related.name} visual`} />
                  <div className="relatedCardTop"><span className="relatedIndex">0{index + 1}</span><span className="relatedTypeLabel">{relatedVisualType}</span></div>
                  <div className="relatedCardCopy"><strong>{related.name}</strong><small>{relatedMicroCopy(relatedVisualType)}</small></div>
        ...
        <div className="serviceCta">
          <div><p className="kicker"><span /> Intex Restoration · Chicagoland</p><h2>{service.emergency ? "Property damage does not wait. Neither should the first call." : "Ready to plan the next step?"}</h2></div>
          <div>
            ...
            <div className="heroActions"><a className="primaryButton" href={`tel:${site.phone}`}>Call {site.phoneDisplay}</a><a className="secondaryButton" href={`mailto:${site.email}`}>Email Intex</a></div>
          </div>
        </div>
      </section>
