...
function visualTypeForSlug(slug: string): ServiceVisualType {
  if (slug === "water-damage-restoration-chicago") return "water";
  if (slug === "fire-damage-restoration-chicago") return "fire";
  if (slug === "mold-remediation-chicago") return "mold";
  if (slug === "trauma-biohazard-cleaning-chicago") return "biohazard";
  if (slug === "insurance-claims") return "claims";
  if (slug === "commercial-restoration-chicago") return "commercial-restoration";
  if (slug.includes("flat")) return "flat";
...
function isRoofingVisual(type: ServiceVisualType) {
  return ["repair", "replacement", "storm", "flat", "commercial", "inspection", "roofing"].includes(type);
}

function relatedMicroCopy(type: ServiceVisualType) {
  const copy: Record<ServiceVisualType, string> = {
    inspection: "Find weak points before they become expensive failures.",
    replacement: "A complete roofing system rebuilt for long-term protection.",
    repair: "Stop active damage and restore the vulnerable part of the roof.",
    storm: "Hail and wind response built around inspection, protection and recovery.",
    flat: "Membrane and low-slope roofing built for Chicago multifamily properties.",
    commercial: "Large-scale roofing planned around operations, access and safety.",
    roofing: "Protect the full roofing system from the top down.",
    water: "Extract water, dry affected materials and control the path of damage.",
    fire: "Coordinate stabilization, smoke and soot cleanup, and restoration scope.",
    mold: "Address moisture, containment and affected materials as one problem.",
    biohazard: "Discreet, controlled cleanup for sensitive property situations.",
    claims: "Document visible conditions and keep restoration scope organized.",
    "commercial-restoration": "Restore damaged areas while planning around tenants and operations.",
  };
  return copy[type];
}
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const visualType = visualTypeForSlug(service.slug);
  const roofingService = isRoofingVisual(visualType);
  const chicagoContext = chicagoContextForSlug(service.slug);
  const providerType = roofingService ? "RoofingContractor" : "Organization";
  const serviceSchema = {
    ...
    provider: { "@type": providerType, name: site.name, url: site.url, telephone: site.phone, email: site.email },
  };
  ...
  return <main className={`servicePage servicePage--${visualType}`}>
      ...
      <div className="serviceHeroStage">
        <div className="serviceHeroInner">
          ...
          <div className="serviceHeroActions">
            ...
            <a className="secondaryButton" href={`/estimate/?service=${service.slug}`}>Start a request <span>↗</span></a>
          </div>
          <div className="serviceHeroTrust">
            <span>{roofingService ? "Chicago roofing specialists" : "Chicago property restoration"}</span>
            <span>Clear scope</span>
            <span>Documented work</span>
          </div>
        </div>
        <RoofingServiceVisual type={visualType} label={`${service.name} visual`} />
      </div>
    </section>
    <section className="serviceBody">
      ...
      <div className="serviceProcess"><p className="kicker dark"><span /> {roofingService ? "A clearer roofing process" : "A controlled restoration process"}</p><h2>{roofingService ? "What happens next" : `How ${service.name.toLowerCase()} moves forward`}</h2><AnimatedProcessSteps steps={service.process} ariaLabel={`${service.name} process`} /></div>
      <div className="relatedServices relatedServicesPremium">
        <div className="relatedServicesIntro"><div><p className="kicker dark"><span /> {roofingService ? "Connected roofing system" : "Connected property recovery"}</p><h2>{roofingService ? <>One roof. Different problems. <em>One smarter next step.</em></> : <>One property. Different damage paths. <em>One coordinated next step.</em></>}</h2></div><p>{roofingService ? "Each service solves a different failure point in the building envelope. Explore the service that matches what your property is actually facing." : "Water, smoke, moisture, roofing, cleanup and documentation can overlap after property damage. Explore the service that best matches the condition in front of you."}</p></div>
        <div className="relatedGrid relatedGridPremium">{service.related.map((relatedSlug,index)=>{const related=serviceBySlug[relatedSlug];if(!related)return null;const relatedVisualType=visualTypeForSlug(related.slug);return <a className={`relatedCard relatedCardPremium relatedCardPremium--${index+1}`} href={`/${related.slug}/`} key={related.slug} data-visual={relatedVisualType}><RoofingServiceVisual type={relatedVisualType} compact label={`${related.name} visual`} /><div className="relatedCardTop"><span className="relatedIndex">0{index+1}</span><span className="relatedTypeLabel">{relatedVisualType.replaceAll("-", " ")}</span></div><div className="relatedCardCopy"><strong>{related.name}</strong><small>{relatedMicroCopy(relatedVisualType)}</small></div><div className="relatedCardAction"><span>Explore service</span><b>↗</b></div></a>})}</div>
      </div>
      <div className="serviceCta"><div><p className="kicker"><span /> Intex Restoration · Chicagoland</p><h2>{service.emergency ? "Property damage does not wait. Neither should the first call." : "Ready to plan the next step?"}</h2></div><div><p>Tell us what happened and where the property is located. For active emergencies, calling is the fastest way to reach the team.</p><div className="heroActions"><a className="primaryButton" href={`tel:${site.phone}`}>Call {site.phoneDisplay}</a><a className="secondaryButton" href={`/estimate/?service=${service.slug}`}>Start request</a></div></div></div>
    </section>
